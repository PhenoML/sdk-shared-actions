...
- [`sdk-release-gate`](.github/workflows/sdk-release-gate.yml) — runs the
  ...
  and validate the prefixed `backend-origin/<originGitCommit>` release tag
  name. Version tags, origin release tags, GitHub releases, package builds,
  and registry publish jobs stay in each SDK repo.
- [`sdk-release-finalize`](.github/workflows/sdk-release-finalize.yml) —
  ...
  Fern Replay is configured, the stable replay generation tag all exist and
  point at the expected commits.
